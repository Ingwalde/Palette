...
def test_verify_invalid_token(client):
    resp = client.get("/api/auth/verify", params={"token": "not-a-real-token"})
...
def test_resend_verification_is_generic(client):
    _register(client)
    known = client.post("/api/auth/resend-verification", json={"email": "carol@test.com"})
    unknown = client.post("/api/auth/resend-verification", json={"email": "nobody@test.com"})
