#!/usr/bin/env bash
# Bring up the real stack, run the integration suite against it, tear it down.
#
#   ./scripts/integration.sh
#
# Unlike the stubbed e2e specs, these need nginx, FastAPI, PostgreSQL and Redis all running and
# talking to each other. Compose owns that, so this script owns Compose — a Playwright
# webServer can start a process but cannot bring a stack up and take it down again.
#
...
DEFAULT_ADMIN_PASSWORD=integration-admin-pass-1
